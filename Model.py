...
def preprocess(text_in_df):
    ...
    for text in text_in_df:
        ...
        text = re.sub(r'(…)( [A-ZẢÔẾỦỞ])', r'\1.\2', text)
        text = re.sub(r'(…)( [a-zảôếủở])', r'\1;\2', text)
    ...
    for j in range(len(df)):
        for i in df["Ticker"].iloc[j]:
            ...
            if i in list(ex_dic_companyname.values()):
                ...
                sen_lst = re.findall('([^@]*'+  r'\b' + str(i) + r'\b' + '[^@]*)', df["Paragraph"].iloc[j]) 
...
def polarity_score(text):
    ...
    if re.search(r'(tăng(( gần)|( đến)|( hơn)|( trưởng)|( từ)|( tới)|( trên))? \d)|(từ.*( lên))|(\D\+ ?\d\.?\d?%)|(mua \d)|(đầu tư( gần)? \d)', text):
        pos_score +=0.05
    neg_score = sum(-0.05 for word in neg if text.lower().find(word) != -1)
    if re.search(r'(giảm(( gần)|( đến)|( hơn)|( tới)|( từ)|( trên))? \d)|(từ.*((xuống)|(còn)))|(\D\- ?\d\.?\d?%)', text):
        pos_score -=0.05
...
if __name__ == '__main__':
    df = input_data('TinChungKhoan.csv')
    # df = df.head(3000)
